...
export interface VeconResult {
  ...
  yCap: number;          // שיעור היוון ריאלי (זהה ל-Vrent)
}
...
export function calcVecon(inputs: VeconInputs): VeconResult {
  ...
  // DCF ריאלי: תזרים ריאלי מהוון בשיעור ריאלי
  // NOI גדל בקצב g ריאלי, מהוון ב-(rf + riskPremium) ריאלי
  const realDiscountRate = rf + riskPremium;
  let pvCashFlows = 0;
  for (let t = 1; t <= HOLD_YEARS; t++) {
    const noi = noiYear1 * Math.pow(1 + g, t - 1);
    pvCashFlows += noi / Math.pow(1 + realDiscountRate, t);
  }

  // שווי שיורי: NOI שנה 11 ÷ yCap, מהוון לשנה 0
  const noiYear11 = noiYear1 * Math.pow(1 + g, HOLD_YEARS);
  const terminalValueFuture = noiYear11 / yCap;
  const terminalValue = terminalValueFuture / Math.pow(1 + realDiscountRate, HOLD_YEARS);
